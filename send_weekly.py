...
START = time.time()
DEADLINE = START + 12 * 60   # a keresésre legfeljebb 12 percet szánunk
RATE_LIMITED = False
...
HU_WORDS |= {"fel", "kezeket", "kezek", "kéz"}
# Jellegzetesen magyar ragok/végződések (legalább 3 betűs tő után)
HU_SUFFIX = re.compile(r"\w{3,}(nék|ném|nél|ként|ról|ről|tól|től)$")
...
def is_hungarian(text):
    ...
    return any(w in HU_WORDS or HU_SUFFIX.match(w) for w in words)
...
def get(url, headers=None, **kw):
    ...
    for attempt in range(5):
        ...
        if r.status_code == 429:
            ...
            if wait > 60:
                RATE_LIMITED = True
                print(f"Spotify sebességkorlát, Retry-After: {wait} mp - a további kéréseket kihagyom.")
                return r
            time.sleep(wait + 1)
            continue
...
def load_names(path):
    """Fájlból nevek: {kisbetűs: eredeti}. Sorok: 'Név' (# = megjegyzés)."""
    out = {}
...
def main():
    ...
    block = set(load_names("blocklist.txt"))   # kézzel kizárt előadók
    albums, errors = {}, 0
    ...
    collect("tag:new", 20)
    for i, w in enumerate(sorted(HU_WORDS), 1):
        collect(f"{w} tag:new", 1)
        if i % 50 == 0:
            print(f"  ... {i}/{len(HU_WORDS)} szó, {int(time.time() - START)} mp, {len(albums)} kiadás")
    ...
    def excluded(al):
        ...
        return (any(e in n for e in EXCLUDE)
                or any(x["name"].lower() in block for x in al["artists"]))
    ...
    detected = {}   # ezen a héten címből/névből felismert előadók
    for al in albums.values():
        ...
        for n in names_of(al):
            if any(c in n for c in STRONG_CHARS):
                detected[n.lower()] = n
    for k in list(detected):
        if k in block:
            del detected[k]
    hu_artists |= set(detected)
